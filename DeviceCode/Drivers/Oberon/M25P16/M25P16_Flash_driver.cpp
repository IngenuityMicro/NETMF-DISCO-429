...
#define M25P16_BLOCK_SIZE 65536


const SPI_CONFIGURATION g_M25P16_spi = {
...
BOOL M25P16_EnableWrite()
{
    NATIVE_PROFILE_HAL_DRIVERS_FLASH();
    
    BYTE en_cmd = WrEn_Instr;
    return CPU_SPI_nWrite8_nRead8(g_M25P16_spi, &en_cmd, 1, NULL, 0, 0);
}
    
BOOL M25P16_WaitReady()
{
    NATIVE_PROFILE_HAL_DRIVERS_FLASH();
    ...
    do {
        if (!CPU_SPI_nWrite8_nRead8(g_M25P16_spi, &rd_cmd, 1, &status, 1, 1)) return FALSE;
...
//--//
    ...
    //
BOOL M25P16_Flash_Driver::ChipInitialize( void* context )
{
    NATIVE_PROFILE_HAL_DRIVERS_FLASH();
    ...
    //
BOOL M25P16_Flash_Driver::ChipUnInitialize( void* context )
{
    NATIVE_PROFILE_HAL_DRIVERS_FLASH();
    ...
    //
const BlockDeviceInfo* M25P16_Flash_Driver::GetDeviceInfo( void* context )
{
    ...
    //   
BOOL  M25P16_Flash_Driver::Read( void* context, ByteAddress StartSector, UINT32 NumBytes, BYTE * pSectorBuff)
{
    NATIVE_PROFILE_HAL_DRIVERS_FLASH();
    ...
    M25P16_WaitReady();
    return CPU_SPI_nWrite8_nRead8(g_M25P16_spi, rd_cmd, 4, pSectorBuff, NumBytes, 5);
}


BOOL M25P16_Flash_Driver::WriteX( void* context, ByteAddress Address, UINT32 NumBytes, BYTE * pSectorBuff, BOOL increment )
{
    ...
    while(NumBytes > 0) {
        ...
        // ready?
        if (!M25P16_EnableWrite()) return FALSE;
        // write sector
        M25P16_WaitReady();        
        if (!CPU_SPI_nWrite8_nRead8(g_M25P16_spi, wr_cmd, len + 4, NULL, 0, 0)) return FALSE;
        Address += len;
        NumBytes -= len;
        len = 256;
        if (!M25P16_WaitReady()) return FALSE;
    }
    ...
    //   This function reads the number of sectors specified from the device.
    //   
BOOL M25P16_Flash_Driver::Write(void* context, ByteAddress Address, UINT32 NumBytes, BYTE * pSectorBuff, BOOL ReadModifyWrite)
{
    NATIVE_PROFILE_PAL_FLASH();
    
    if (ReadModifyWrite) {
        ByteAddress block = (ByteAddress)(Address & ~(M25P16_BLOCK_SIZE - 1));
        ByteAddress end = (ByteAddress)(Address + NumBytes);
        do {
            if (!EraseBlock(context, block)) return FALSE;
            block += M25P16_BLOCK_SIZE;
        } while (block < end);
    }
...
BOOL M25P16_Flash_Driver::Memset(void* context, ByteAddress Address, UINT8 Data, UINT32 NumBytes)
{
    NATIVE_PROFILE_PAL_FLASH();
...
BOOL M25P16_Flash_Driver::GetSectorMetadata(void* context, ByteAddress SectorStart, SectorMetadata* pSectorMetadata)
{
    // no metadata
    return FALSE;
}

BOOL M25P16_Flash_Driver::SetSectorMetadata(void* context, ByteAddress SectorStart, SectorMetadata* pSectorMetadata)
{
    // no metadata
    return FALSE;
}
    ...
    //    
BOOL M25P16_Flash_Driver::IsBlockErased( void* context, ByteAddress BlockStart, UINT32 BlockLength )
{
    NATIVE_PROFILE_HAL_DRIVERS_FLASH();
    ...
    //    
BOOL M25P16_Flash_Driver::EraseBlock( void* context, ByteAddress Sector )
{
    NATIVE_PROFILE_HAL_DRIVERS_FLASH();
    
    if (IsBlockErased(context, Sector, M25P16_BLOCK_SIZE)) return TRUE;
    ...
    M25P16_WaitReady();
    if (!CPU_SPI_nWrite8_nRead8(g_M25P16_spi, er_cmd, 4, NULL, 0, 0)) return FALSE;
    return M25P16_WaitReady();
}
    ...
    //
void  M25P16_Flash_Driver::SetPowerState( void* context, UINT32 State )
{
...
UINT32 M25P16_Flash_Driver::MaxSectorWrite_uSec( void* context )
{
    NATIVE_PROFILE_PAL_FLASH();
...
UINT32 M25P16_Flash_Driver::MaxBlockErase_uSec( void* context )
{
    NATIVE_PROFILE_PAL_FLASH();
...
#pragma arm section rodata = "g_M25P16_Flash_DeviceTable"
#endif

struct IBlockStorageDevice g_M25P16_Flash_DeviceTable = 
{                          
    &M25P16_Flash_Driver::ChipInitialize,
    &M25P16_Flash_Driver::ChipUnInitialize,
    &M25P16_Flash_Driver::GetDeviceInfo,
    &M25P16_Flash_Driver::Read,
    &M25P16_Flash_Driver::Write,
    &M25P16_Flash_Driver::Memset,    
    &M25P16_Flash_Driver::GetSectorMetadata,
    &M25P16_Flash_Driver::SetSectorMetadata,
    &M25P16_Flash_Driver::IsBlockErased,
    &M25P16_Flash_Driver::EraseBlock,
    &M25P16_Flash_Driver::SetPowerState,
    &M25P16_Flash_Driver::MaxSectorWrite_uSec,
    &M25P16_Flash_Driver::MaxBlockErase_uSec,    
};
