...
{
    UINT16 sr = USART1->SR & USART1->CR1;
...
{
    UINT16 sr = USART2->SR & USART2->CR1;
...
{
    UINT16 sr = USART3->SR & USART3->CR1;
...
{
    UINT16 sr = USART6->SR & USART6->CR1;
    if (sr & USART_SR_RXNE) STM32F4_USART_Handle_RX_IRQ(5, USART6);
    if (sr & USART_SR_TXE)  STM32F4_USART_Handle_TX_IRQ(5, USART6);
}

#if TOTAL_USART_PORT > 6
void STM32F4_USART_Interrupt6 (void* param)
{
    UINT16 sr = UART7->SR & UART7->CR1;
    if (sr & USART_SR_RXNE) STM32F4_USART_Handle_RX_IRQ(6, UART7);
    if (sr & USART_SR_TXE)  STM32F4_USART_Handle_TX_IRQ(6, UART7);
}

void STM32F4_USART_Interrupt7 (void* param)
{
    UINT16 sr = UART8->SR & UART8->CR1;
    if (sr & USART_SR_RXNE) STM32F4_USART_Handle_RX_IRQ(7, UART8);
    if (sr & USART_SR_TXE)  STM32F4_USART_Handle_TX_IRQ(7, UART8);
}
#endif
...
BOOL CPU_USART_Initialize( int ComPortNum, int BaudRate, int Parity, int DataBits, int StopBits, int FlowValue )
{
    ...
    } else if (ComPortNum == 0) { // COM1 on APB2
        RCC->APB2ENR |= RCC_APB2ENR_USART1EN;
        clk = SYSTEM_APB2_CLOCK_HZ;
#if TOTAL_USART_PORT > 6
    } else if (ComPortNum > 5) { // COM7-8 on APB1
    ...
    if (ComPortNum >= 3) alternate = 0x82; // AF8 = UART4-8
    ...
    case 5: CPU_INTC_ActivateInterrupt(USART6_IRQn, STM32F4_USART_Interrupt5, 0); break;
#if TOTAL_USART_PORT > 6
    case 6: CPU_INTC_ActivateInterrupt(UART7_IRQn, STM32F4_USART_Interrupt6, 0); break;
    case 7: CPU_INTC_ActivateInterrupt(UART8_IRQn, STM32F4_USART_Interrupt7, 0);
#endif
    }
...
BOOL CPU_USART_Uninitialize( int ComPortNum )
{
    ...
    case 5: CPU_INTC_DeactivateInterrupt(USART6_IRQn); break;
#if TOTAL_USART_PORT > 6
    case 6: CPU_INTC_DeactivateInterrupt(UART7_IRQn); break;
    case 7: CPU_INTC_DeactivateInterrupt(UART8_IRQn);
#endif
    }
