...
bool CLR_DBG_Debugger::CheckPermission( ByteAddress address, int mode )
{
    ...
    BlockStorageList::FindDeviceForPhysicalAddress( &device, address, sectAddress );
    device->FindRegionFromAddress( address, regionIndex, rangeIndex );
    const BlockRange& range = device->GetDeviceInfo()->Regions[ regionIndex ].BlockRanges[ rangeIndex ];
...
bool CLR_DBG_Debugger::AccessMemory( CLR_UINT32 location, UINT32 lengthInBytes, BYTE* buf, int mode )
{
    ...
    {
        device->FindRegionFromAddress( location, iRegion, iRange );
                ...
                switch(mode)
                {
                    ...
                    case AccessMemory_Read:
                        ...
                        else
                        {
                            ...
                            success = device->Read( accessAddress , NumOfBytes, (BYTE *)bufPtr );
                        ...
                        break;
                    case AccessMemory_Write:
                        success = device->Write( accessAddress , NumOfBytes, (BYTE *)bufPtr, FALSE );
                        break;

                     case AccessMemory_Erase:
                        success = device->EraseBlock( accessAddress );
                        break;
                     default:
                        break;
                }
